...
| `get <key>` | `GET /v1/accounts/{account-key}` | — | 詳細ビュー (key-value 形式) |
| `summary <key>` | `GET /v1/accounts/{account-key}/summary` | — | 残高・サブスク数・請求サマリー |
| `payment-methods <key>` | `GET /v1/accounts/{account-key}/payment-methods` | — | TYPE, LAST4, DEFAULT, STATUS |
| `payment-methods-default <key>` | `GET /v1/accounts/{account-key}/payment-methods/default` | — | 詳細ビュー |
| `payment-methods-cascading <key>` | `GET /v1/accounts/{account-key}/payment-methods/cascading` | — | 詳細ビュー |

### subscription コマンド (Read)

| サブコマンド | API | フラグ | テーブル出力カラム |
|---|---|---|---|
| `list --account <key>` | `GET /v1/subscriptions/accounts/{account-key}` | `--account` (必須, URL パスに埋め込み), `--page-size`, `--charge-detail` | ID, NUMBER, NAME, STATUS, TERM_TYPE, START, END |
| `get <key>` | `GET /v1/subscriptions/{subscription-key}` | — | 詳細ビュー |
| `versions <key> <ver>` | `GET /v1/subscriptions/{subscription-key}/versions/{version}` | — | 詳細ビュー |
