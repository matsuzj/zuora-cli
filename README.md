...
zr data-query submit "SELECT * FROM invoice"   # async — prints a job id
zr data-query get <job-id>                      # status + result URL
zr data-query list --status completed           # recent jobs
...
| `invoice` | Invoice list + get + items + files + email + usage-rate-detail + post/reverse/writeoff |
...
| `data-query` | Zuora Data Query (async SQL): submit / get / list / cancel / run (poll + S3 result download) |
...
-v, --verbose count        Verbose output (request/response diagnostics);
    ...
    --read-only-allow-data-query
                           In read-only mode, also allow Data Query submit/cancel
                           (POST /query/jobs, DELETE /query/jobs/{id}); other writes
                           stay blocked. Also via ZR_READ_ONLY_ALLOW_DATA_QUERY=1
    --timeout <dur>        Abort if the command runs longer than this (e.g. 30s, 2m; 0 = no limit)
```
...
**Command timeout**: `--timeout <duration>` (e.g. `--timeout 30s`) bounds the **whole** command — every retry and backoff, not just one request — so a non-interactive run (CI/script) can't hang on persistent rate-limiting where Ctrl-C isn't available. A deadline exits with code 1 (distinct from Ctrl-C's 130). The default `0` means no limit.

**Mutation audit trail**: there is no built-in audit-log file, but every request and response (method, URL, and all headers — including the `Idempotency-Key` on POST/PATCH, with `Authorization` always masked as `Bearer ***`) is written to **stderr** under `-v`, so you can capture a persistent record by redirecting stderr:

```sh
zr subscription create --account-key A-001 --body @sub.json -vv 2>>~/zuora-mutations.log
```
...
`ZR_READ_ONLY_ALLOW_DATA_QUERY` is a **separate knob** from `ZR_READ_ONLY`, not a replacement: `ZR_READ_ONLY` *enables* read-only mode in the first place, whereas `ZR_READ_ONLY_ALLOW_DATA_QUERY` only *relaxes* it for Data Query and has no effect unless read-only is already on. Both fail closed, but in **opposite** directions — an unrecognized `ZR_READ_ONLY` value turns read-only **on** (more restrictive), while an unrecognized `ZR_READ_ONLY_ALLOW_DATA_QUERY` value leaves it **off** (does not widen the guard). For each, the corresponding flag takes precedence over the env var.

| | `ZR_READ_ONLY` (`--read-only`) | `ZR_READ_ONLY_ALLOW_DATA_QUERY` (`--read-only-allow-data-query`) |
|---|---|---|
| Effect | Turns read-only mode **on** (blocks writes) | When read-only is on, also allows **Data Query** submit/cancel |
| Default | off | off |
| Unknown value | → **on** (fail closed = block) | → **off** (fail closed = don't widen) |

<!-- destructive-list:begin — generated by scripts/gen-destructive-list.sh; make lint fails on drift -->
**Destructive operations**: irreversible commands require an explicit `--confirm` flag: `zr account delete`, `zr billrun cancel`, `zr billrun delete`, `zr billrun post`, `zr contact delete`, `zr contact scrub`, `zr data-query cancel`, `zr fulfillment delete`, `zr fulfillment-item delete`, `zr invoice post`, `zr invoice reverse`, `zr invoice writeoff`, `zr omnichannel delete`, `zr order cancel`, `zr order delete`, `zr order delete-async`, `zr order revert`, `zr payment cancel`, `zr payment refund`, `zr prepaid deplete`, `zr prepaid reverse-rollover`, `zr subscription cancel`, `zr subscription delete`, `zr usage delete`.
<!-- destructive-list:end -->
...
End-to-end suites (the real binary against a live Zuora tenant) live in
`tests/e2e-*.sh` (`./tests/run-all.sh` after `zr auth login`); skips are
catalogued in [docs/e2e-test-skips.md](docs/e2e-test-skips.md).
