...
func runGet(cmd *cobra.Command, f *factory.Factory, debitMemoID string) error {
	...
	resp, err := client.Get(fmt.Sprintf("/v1/debitmemos/%s", url.PathEscape(debitMemoID)), api.WithCheckSuccess())
	if err != nil {
		return err
	}
