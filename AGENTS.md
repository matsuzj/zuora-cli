...
- Implementation plans live in `docs/plans/` — the template, the lifecycle
  (`Status: active|shipped (PR #n)`, drift-gated by `make lint`; archiving = move
  to `docs/plans/archive/` with the historical banner), and
  the multi-pass review protocol are defined in `docs/plans/README.md`. Read it
  before writing or reviewing a plan; flip Status + append the implementation
  memo (deviations) in the PR that ships the plan.
...
- **Probe contract (#524)**: read probes run `zr api` with `--read-only`; a probe that WRITES to the tenant (e.g. creating a Draft order to probe a response shape) must first pass `scripts/require-sandbox.sh` — the same fail-closed tenant gate the E2E suites use (#267), runnable standalone.
- **Before auditing response shapes, read `docs/audits/verified-non-issues.md`** — live-probed ground truth and confirmed false positives (do not re-flag them); new confirmed non-issues get appended there with the probe method and date (#525).
- A doc-verified-but-not-live-verified assumption gets a `LIVE-UNVERIFIED(<what>; since YYYY-MM-DD; trigger: <what unblocks it>)` comment at the assumption site. `make pending-live` is the registry (generated into `docs/e2e-test-skips.md`, drift-gated by lint). **A PR-body deferral must reference an open issue or such a marker — prose alone evaporates into merged PRs (#521).** Resolve by live-probing, replacing the marker with a dated `live-verified against <tenant> (<date>)` comment, and regenerating.
- E2E: some checks legitimately skip on the sandbox (unprovisioned features); see `docs/e2e-test-skips.md`.
- E2E suite quality bar (merged from the retired review checklist): suites CREATE their own test data (never depend on pre-existing tenant state); include validation checks (missing required flags/args); include `--json`/`--jq` output-format checks; and skips are status-specific pass/skip decisions per tenant constraint — never blanket skips.
