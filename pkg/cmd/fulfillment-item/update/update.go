...
Examples:
  zr fulfillment-item update 2c92c0f8... --body @update.json
...
func runUpdate(cmd *cobra.Command, opts *updateOptions, itemID string) error {
	...
	resp, err := client.Put(fmt.Sprintf("/v1/fulfillment-items/%s", url.PathEscape(itemID)), bodyReader, api.WithCheckSuccess())
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(f.IOStreams.ErrOut, "Fulfillment item %s updated.\n", itemID)
	return nil
}
