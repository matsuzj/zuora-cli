...
func newTestRoot(f *factory.Factory) *cobra.Command {
	...
	sub.AddCommand(NewCmdTransfer(f))
	root.AddCommand(sub)
	return root
}
...
func TestContactTransfer_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "/v1/contacts/c-123/transfer", r.URL.Path)
		w.WriteHeader(200)
	...
	root := newTestRoot(f)
	root.SetArgs([]string{"contact", "transfer", "c-123", "--body", `{"destinationAccountId":"a-2"}`})
	require.NoError(t, root.Execute())
	assert.Contains(t, out.String(), "true")
	assert.Contains(t, errOut.String(), "Contact c-123 transferred.")
}
...
func TestContactTransfer_RequiresBody(t *testing.T) {
	...
	root := newTestRoot(f)
	root.SetArgs([]string{"contact", "transfer", "c-123"})
