...
**Destructive operations**: irreversible commands require an explicit `--confirm` flag. This includes `delete` for `account` / `contact` / `order` / `subscription` / `usage` / `fulfillment` / `fulfillment-item` / `omnichannel`, plus `order cancel`, `order revert`, `subscription cancel`, `contact scrub`, and `prepaid reverse-rollover`.
