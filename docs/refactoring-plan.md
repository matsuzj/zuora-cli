...
> **この文書が現行の進行管理を担う**(2026-06-13、P7-1)。各フェーズ末尾の
> 実装メモが完了記録。初期開発プラン(docs/plans/)は archive/ に移動済み。
...
- **実装メモ(2026-06-11)**: 実装完了。lib +155行 / スイート -526行・委譲呼び出し +41行(正味 -330行)。1スイート1コミット、全9本をライブ実行しテスト行がベースラインログと diff 一致することを確認。全スイートが `_drain_log`(ログ切り捨て修正)と `ZR_E2E_RATE_PLAN_ID` override を獲得。print_summary 正規化により e2e-subscription-write のサマリーのみ出力形式が変化(条件付き色付き行 → 常時表示の無色行、他8本と同形)。既知quirk として e2e-subscription-write の `ZR="./bin/zr"`(CWD相対)はコメント明記の上で温存(別PR向け)。**フェーズ0 完了**。
...
- **実装メモ(2026-06-11)**: 6コミットで実装完了(抽出+characterization → rootCmd導出 → shlex → Load警告+パス一本化 → set側ガード → E2E)。計画との差分: ガードは予約名+自己参照に加え**不正クォートと空展開も set 時に拒否**(broken エイリアスを作れなくした)。`alias delete` は許容のまま(テストで固定)。e2e-local 35/35。パス一本化は **Codex レビューで撤回**: 展開を `f.Config()` 成功にゲートすると config 破損時に(config 不要なコマンドへの)エイリアスまで黙って死ぬ回帰になるため、展開はパース不要の純粋なパス解決 `config.Dir()` を維持(main.go にコメントで理由を明記)。P4-1 の「config.Dir() 削除」は**取り消し**(正当な呼び出しが1件残る)。**P1-3 完了**。
...
- **実装メモ(2026-06-11, P1-4)**: cmdutil.SleepContext 新設(client.go の sleepWithContext は P4 整理まで併存)、job-status --watch は ctx 対応 + `--interval`/`--timeout`(既定 5s/0=無制限で現挙動維持、timeout 超過は「gave up waiting」エラー)。login→ForceRefreshContext / token→TokenContext(cmd.Context())。bite 証明: 旧実装で auth 2テストがちょうど30秒で fail、watch テストは無限ループでタイムアウト。テスト知見: ハングサーバーのハンドラは r.Context().Done() でなくテスト所有チャネルで解放(POST ボディ未読だと切断が ctx に伝わらず server.Close() がデッドロック)。**P1-4 完了 = フェーズ1 完了**。
...
- **実装メモ(2026-06-11, P3-1 ランナー)**: cmdutil.Action / RunDetail を実装(別PR)。設計どおり SuccessMsg は func(raw) string(空文字=抑制 — payment/create の id 条件パターン対応)、エラーラップ "parsing response: %w" は既存テスト互換のため逐語維持、Fprint(Fprintf でなく — 動的値の % を書式扱いしない)。success チェックはランナー経由でもデフォルト適用(テストで固定)。移行第1号は contact/get を推奨(純GET・フラット11フィールド)。**→ 実施済み(2026-06-11)**: get.go -33/+19、既存テスト3本が無変更で PASS(プロトコル実証)。
- 移行手順: ランナー+単体テストを先に独立PRで → 1リソース=1PR で機械移行。**既存 _test.go は変更せずグリーン維持**(`parsing response: %w` の文言まで含めて互換)。最初のリソース移行後と全移行後に E2E。
- **実装メモ(2026-06-11〜12, P3-1 完了)**: contact/get を起点に **75ファイル**を 1リソース=1PR で cmdutil.RunDetail へ移行完了(PR #76〜#150。全PR: 既存テスト無変更グリーン + make ci + GitHub CI + Codex クリーン + フル live E2E をマージ前に通過)。実証された Action の適用範囲: 純GET(パスパラメータ1〜2、最大11フィールド)/ネスト unwrap(order・ramp — フォールバック付き unwrap をクロージャ内へ逐語移動)/POST・PUT+body/bodyless PUT・POST・DELETE/任意 body(caller 構築の WithBody を ReqOpts 経由 — invoice/writeoff)/WithQuery×4(meter/audit)/multipart(usage/post — フォーム合成は caller 側、Content-Type は ReqOpts)/caller 側 body 合成(subscription ライフサイクル・meter/summary)/id・jobID ガード付き&2行連結 SuccessMsg(空文字=抑制)/bulk 配列 peek(fulfillment・fulfillment-item create — ファイル内純ヘルパー firstFulfillment/firstItemID に抽出し Fields・SuccessMsg 両クロージャから呼ぶ)。挙動保存を厳守: 監査済みの応答キー疑義(order-line-item/get、fulfillment update 系)は修正せずそのまま移行(未承認のため)。**手書き例外(理由付き)**: account/get・account/summary(ネスト+専用整形 — 計画どおり)、order/job-status(--watch ポーリングループは1リクエスト1描画の Action 形に非適合 — 新規確定)、version(非API)。charge/get・plan/get・product/create ほか commerce 系 JSON-only テール28ファイルと delete 系10ファイルは計画どおり P3-3 残置。運用知見: (a) 無引数 SuccessMsg は fmt.Sprintf でなくプレーン文字列を返す(staticcheck S1039)、(b) GitHub が pull_request の CI run を生成しないことが稀にある — close→reopen でイベント再発火(パイプラインに自動検知を組込み)、(c) zoql E2E の query-CSV 空ページフレークが2回 — スタンドアロン再実行で green 確認→フル E2E 再実行→マージの手順で対処。**P3-1 完了**。
...
- **実装メモ(2026-06-12, P3-2 完了)**: ランナー(PR #152、ユーザー承認後マージ)+ **11 list リソース全件移行完了**(PR #184〜#195: creditmemo → debitmemo → invoice → payment → order → order list-by-subscription/invoice-owner/subscription-owner/list-pending → subscription → account。全PR: 既存テスト無変更グリーン + make ci + GitHub CI + Codex クリーン + フル live E2E)。ランナー実装は計画の補正点をすべて反映: 型付き Flag(Int+IntDefault は常時送信 — account --page-size 20 / Repeatable→WithQuerySlice / Query="" の path-only)、Path(args, flags)、ColumnSpec.Money(%.2f + 欠損キーは型付き struct のゼロ値 "0.00")、正準 nextPage ヒント(page 式=URL から抽出 / cursor 式=逐語、弱ヒントフォールバック、シェル単引用エスケープ — レビューで strconv.Quote の $-展開脆弱性を検出して置換、制御/Cf 文字は空白化)。非配列 ItemsKey はエラー(型付き strictness 維持)。**意図した辺縁変更2点**(テスト無変更で green): 弱ヒント勢の正準ヒント昇格(計画の統一意図)と余剰位置引数の拒否(NoArgs デフォルト)。**テストのユニオン移行は未実施** — 既存テスト無変更を移行の等価性証明に使ったため、テスト再編は P3-4 に送る。contact/list は対象外(計画どおり)。レビュー: Codex 計4パス + 敵対的 WF 25 エージェント(17確認 → major 2修正: 非配列黙殺・ヒント端末注入)。
...
- **実装メモ(2026-06-12, --csv 明示エラー + delete 統一 — 両方ユーザー承認済み)**: (1) **--csv 明示エラー化完了**(PR #197): `output.RenderJSONOnly`(read形28本中21本はこれで終端)+ `output.RejectBareCSV`(write形7本はリクエスト前に拒否 — Codex が2ラウンドで「ミューテーション実行後の拒否は二重作成を誘発」を指摘し、全28本を**リクエスト前 fail-fast** に統一)。README の優先順位(--jq/--json/--template が --csv に勝つ)は保存 — 拒否は**素の --csv のみ**(#54 の教訓をテストで固定)。文言は zr api 前例に整合。リリースタグに挙動変更として記載予定。(2) **delete 全8本を RenderDeleteResult に統一**(PR #198/#199/#200、B案承認): ヘルパーが JSON detail 後にも humanMsg を出すよう拡張(RunDetail.SuccessMsg と同慣例、Fprint で %-動詞無害化+回帰テスト)。統一ポリシー = 204/空200/非JSON200=成功・parse失敗は "parsing response: %w"。**billrun delete の「204 で必ず parse エラー」潜在バグを修正**。UnparseableBody テスト4本は統一ポリシーに更新。
- **実装メモ(2026-06-12, P3-3 挙動保存サブセット完了)**: JSON-only テール **28ファイル全件**を 1コマンド=1PR で `output.RenderJSON` 経由に置換完了(PR #153〜#180。全PR: 既存テスト無変更グリーン + make ci + GitHub CI + Codex クリーン + フル live E2E)。置換形は2種: **read 形**(21ファイル — `if handled, err := output.RenderJSON(...); handled || err != nil { return err }` + 明示デフォルト PrintJSON フォールスルー)と **write 形**(7ファイル: charge create/update/update-tiers, plan create/update, product create/update — デフォルト出力後に stderr 成功メッセージがあり、**--json でもメッセージが出るのが現行挙動**(RunDetail.SuccessMsg と整合)のため `if fmtOpts.JQ != "" || fmtOpts.Template != ""` ガード越しに RenderJSON へ委譲。JSON 分岐まで畳むとメッセージが消えて挙動が変わる)。全フラグ組合せでバイト同一(--json+--template は root で拒否済み、--jq 優先は新旧一致)。**未実施(別判断)**: (1) `--csv` 明示エラー化 — 挙動変更なので中央集約(RenderJSON 側)+ CHANGELOG + ユーザー承認で別PR、(2) delete系7ファイルの RenderDeleteResult 化 — 空ボディ/非JSON 挙動の等価性をファイル毎に精査要、(3) ~~query.go の冗長プレディスパッチ削除~~ → **実施済み(PR #182)**: 完全削除は --jq/--json/--template と query 自前の --csv 併用時に優先順位が反転する(現行は JSON 系優先、削除すると CSV が先勝ち)ため、**削除でなくその場で RenderJSON 置換**(JQ>JSON>Template の順序が分岐と完全一致 — バイト同一)。version.go は対象外(独自 JSON 構造)。
...
- **実装メモ(2026-06-12, P3-4 完了)**: パイロット(contact/get 手書き、PR #201)+ **4波の並列エージェント移行**(PR #202: 18本 / #203: 31本 / #204: 42本 / #205: 45本)で **140ファイル中135本+部分2本(api/config の適合サブセット)を cmdtest へ移行完了**。手法: ファイル単位の worktree 分離エージェント(各自 gofmt/vet/test/名前パリティ自検証)→ 中央で再検証(go test -list の前後 diff によるケース黙失防止 + 全ツリー green)→ 波単位 PR + フルゲート。**理由付き温存3本**: alias(ファイルシステム永続化断定)、auth(トークン事前注入 config)、config/set(MockConfig 注入+状態断定)— 非HTTP ハーネス需要のため bespoke を維持。**ハーネス強化**: cmdtest.Run が ambient ZR_READ_ONLY を t.Setenv で中和(Codex 指摘 — 実 globalflags を通すため、安全設定として常時 export しているマシンで write 成功テストが全滅する問題。ZR_READ_ONLY=true 下で検証済み)。マルチリクエスト(pagination/watch)やボディ断定はカスタム http.HandlerFunc を Run に渡す形で全数保存。**P3-4 完了 = フェーズ3 全項目完了**。
...
- **実装メモ(2026-06-13, フェーズ5 完了 — PR #223〜#227、承認済み5PR計画・フルゲート自動マージ)**: 事前に4レンズ調査で全インベントリを実測(計画の「subscription/list:123」参照は陳腐化 — ヒント補間は listcmd.go の printHint が Spec のフラグ名に自動追従)。**P5-3a**(#223): Example: フィールド移行 132/132 ファイル(単一パススクリプト、手動ゼロ、ヘルプ出力は不変)+ listcmd.Spec.Example / **Flag.DeprecatedName**(旧綴り=同一ポインタの隠し非推奨エイリアス。Required 併用時は cobra の Changed ビット検査がエイリアスを見ないため、run() が**値**を cobra と同文言で強制)+ make lint の Examples: 残存ゲート(bite 検証済み)。AddBodyFlag の required 配線は意図的に P5-2 へ(cobra 検査が手書きガードより先に発火し文言が早期に変わるため)。**P5-1**(#224): invoice/payment/subscription list --account→**--account-key**(Spec DeprecatedName)/commitment list・periods --account→**--account-number**(新 cmdutil.AddAccountNumberFlag)。creditmemo/debitmemo/contact は既に正確で不変。旧綴りは v0.5.x 維持・**v0.6.0 で削除**(承認済み)。E2E に「エイリアス動作+非推奨警告」の live チェック追加。**P5-2**(#225): AddBodyFlag(required)→MarkFlagRequired 配線、手書き --body ガード47件削除、単独フラグ6件(--plan/--file/--run-type/--export-type/--from/--to)→MarkFlagRequired、cancel/suspend/resume→MarkFlagsOneRequired("body","policy")。エラー文言は cobra 標準 `required flag(s) "body" not set` へ(ユニット45+E2E 41行を同コミット更新。live E2E がエスケープ形 grep の取りこぼし3件を検出→修正)。**Codex が実エッジ2件検出**: 明示空値(--body "" / --policy ""、未設定シェル変数で現実的)が cobra 検査を通過 → ResolveBody("") をフェイルファスト化+3コマンドに値レベル分離ガード復元(旧 TestResolveBody_Empty は壊れた契約を固定していたため書き換え)。**意図的除外**: 条件付き必須6件(--policy 値依存)は cobra API が存在せず手書き維持/charge・plan get --key(P5-3cで positional 化)/commitment の --account-number(非推奨エイリアス保持中は cobra 群APIが Changed ビットでエイリアスを拒否するため v0.6.0 まで手書き)。**P5-3b**(#226): cmdutil.EnumCompletion/EnvNamesCompletion + listcmd.Flag.Enum。--policy×3・--periods-type×2・memo --status×2・--env(環境名)・config get/set キー・alias delete 名の計11登録。**逸脱**: meter --run-type/--export-type は正値リストが codebase に無く誤補完回避のため見送り。e2e-local に __complete 3本。**P5-3c**(#227): charge/plan get positional 化(--key は隠し非推奨・RangeArgs(0,1)、無引数エラーは cobra ExactArgs 文言を先取り)/query の --csv ローカルシャドー削除(挙動同一、globalflags の #214 ワークアラウンドコメント更新・回帰テスト不変)/page-size 10コマンド string→Int + Flag.OmitZero(0=送信しない。非数値はクライアント側拒否に変化)/プレースホルダ統一(<meter-id>×5、<usage-id>×3、subscription number 制約文×3)。**完了条件充足**: ヘルプ一様(Example フィールド+lint ゲート)、E2E 10/10 グリーン、ユーザー可視変更は v0.5.0 タグ本文に列挙(CHANGELOG ファイルは無し=タグ本文方式)、リリース v0.5.0。
...
- **実装メモ(2026-06-13, フェーズ6 完了 — PR #229〜#232、承認済み4PR計画・フルゲート自動マージ)**: **P6-1**(#229): Client.vlogf(`* ` 流儀)+ doWithRetry の全判断点に7行(jitter込み実待機・429±Retry-After・401リフレッシュ開始/完了・冪等5xx・transport×2)。採用デフォルト: 毎attempt行は出さない(gh流儀)。必須の漏えいガード TestVerbose_TokenRefreshMasksBearer(リフレッシュ後トークン値の不在+`Bearer ***` 固定文字列)。**P6-2**(#230): TokenSource.Logf(nilガード)を factory.AuthLogWriter 経由で注入(遅延 closure が呼び出し時に読むため Apply の設定が AuthToken/401強制リフレッシュ両経路に効く)。漏えい否定テスト(secret/token/client_id の不在)。**カバレッジラチェット(P4-4)が factory 48.6% を検出**→配線テスト追加で62.9%。**Codex が資格情報ソース誤表示を検出**: login はフラグ/プロンプト由来の StaticCredentialStore を渡すため「keyring」表示が嘘になる→3分岐(OS keyring/env ペア/明示指定値)。**P6-3**(#231, Codex 4ラウンド全て実所見): --verbose を CountP 化(-v/-vv)、ZR_DEBUG=api はレベル1+2 を含意(globalflags.VerboseLevels が単一ゲート)。4KBキャップ+切詰めマーカー、multipart は **大文字小文字非区別**で Content-Type 判定(Codex)、リクエストボディはリトライ用バッファを再利用(追加読みゼロ)。**retry層で消費される 429/401/5xx ボディも -vv で出す**(readAPIError を Client メソッド化 — 診断価値が最も高いケースが漏れていた; Codex)。auth login/token も VerboseLevels を共有(ZR_DEBUG 単独で auth 行が出ない非対称を Codex が検出)。alias 展開の arity 導出に count 型を追加。cmdtest は ZR_DEBUG を中和。**P6-4**(#232): ZR_ENV を --env の既存検証パスに実装(フラグ > ZR_ENV > config、exact-match — 未知名は --env と同一エラーで黙殺しない)。cmdtest は ZR_ENV も中和。context.Canceled → exit 130(exitCoder より先に判定、wrap 対応)。README 優先順位表+環境選択文を更新。live E2E 2本(未知名 fails-loudly / 有効名 — Codex 指摘で run_retry 化)。
...
- **実装メモ(2026-06-13, フェーズ7 完了 — PR #234/#235 + 本メモ。これで計画全7フェーズ完走)**: **P7-1**(#234): 新規 docs/architecture.md(宣言的ランナーを正準とする現行構造リファレンス)を AGENTS.md の参照先に; phase-*.md + plans/README.md(計12文書)を docs/plans/archive/ へ歴史的バナー付きで移動; phase-05-pending.md(生きたTODO)は e2e-test-skips.md の「Commerce live verification」節に統合して削除; zuora-api-reference.md は read-only 調査エージェントでコード突合 — 概ね正確、既知のずれ4点(invoice list のパス族 / delete-async は PUT / lifecycle の {} ボディ / object-query account 未実装)+ 3月以降の未記載群をバナーに記録; review-checklist の参照差し替え; 本計画書がトラッカーであることを冒頭に宣言; read-only-mode.md は現役のため据え置き。**P7-2**(#235): AGENTS.md を貢献者ワークフローの単一情報源化(README Development は前提+make ci+ポインタに縮約); 実矛盾修正(test 行のカバレッジフラグ欠落、「9 suites」リテラル→glob); README の破壊的コマンドリストを scripts/gen-destructive-list.sh 生成+マーカー区間+make lint ドリフトゲートに置換(bite 検証済み — 手書きリストは現時点19件で偶然一致していたが恒久化); P3〜P6 の新規約を AGENTS.md に記録(宣言的ランナー必須・options構造体・フラグ語彙・生成リスト運用)。**(任意項目の帰結)**: exit-130 は P6-4 で実装済み。**計画完了条件**: 全7フェーズの実装メモが揃い、E2E 10/10 グリーン、v0.4.0/v0.5.0 をリリース済み。残る将来義務は v0.6.0 での非推奨エイリアス削除(--account/--key、listcmd.DeprecatedName、commitment の cobra 群API化、charge/plan get の ExactArgs 化)のみ。

> **2026-07-08 追記(全義務完了)**: 上記の非推奨エイリアス削除義務は **#291(v0.7.0, 2026-06-18)で消化済み**。その後 #512/#513(v0.9.0, 2026-07-06)が -by-X コマンド群と `--timeout`/`--export` フラグエイリアスも削除(`usage post` は意図的に存続 — tests/ の ALIAS-TRIPWIRE 参照)。本計画に未消化の義務は残っていない。この文書は完了済みの歴史的記録である。
---
