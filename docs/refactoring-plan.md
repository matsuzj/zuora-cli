...
- **実装メモ(2026-06-11, P2-2)**: RenderJSON(JQ>JSON>Template、(handled,error) 戻り値で fall-through 表現)/ RenderSuccess / cmdutil.RenderDeleteResult(204・空200・非JSON200 = 成功、JSON ボディは detail 描画)を新設し、Render/RenderDetail の先頭3分岐を RenderJSON に畳んだ(バイト互換、既存テストで固定)。28ファイルの尾部置換と --csv 明示エラーは計画どおり P3-3。**json.go 内部 dedup(prettyJSON/emptyBody/sanitize 統合 約27行)は未実施 — P2-2 残作業**として次イテレーション以降で。
