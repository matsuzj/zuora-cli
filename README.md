...
**Data Query in read-only mode**: Zuora Data Query is read-only by nature but submits via `POST /query/jobs` and cancels via `DELETE /query/jobs/{id}`, so under `--read-only` it is **blocked by default**. Pass `--read-only-allow-data-query` (or set `ZR_READ_ONLY_ALLOW_DATA_QUERY=1`) to permit Data Query submit/cancel while every other write stays blocked. The flag takes precedence over the env var, and the env var fails closed in the safe direction (an unrecognized value does **not** enable it).
