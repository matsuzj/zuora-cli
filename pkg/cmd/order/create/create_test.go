...
func newTestRoot(f *factory.Factory) *cobra.Command {
	...
	order.AddCommand(NewCmdCreate(f))
...
func TestOrderCreate_Success(t *testing.T) {
	...
	root := newTestRoot(f)
	root.SetArgs([]string{"order", "create", "--body", `{"existingAccountNumber":"A001"}`})
	err := root.Execute()

	require.NoError(t, err)
	assert.Contains(t, out.String(), "O-00000001")
	assert.Contains(t, errOut.String(), "Order O-00000001 created.")
}
...
func TestOrderCreate_SuccessFalse(t *testing.T) {
	...
	root := newTestRoot(f)
	root.SetArgs([]string{"order", "create", "--body", `{"bad":"data"}`})
	err := root.Execute()
...
func TestOrderCreate_RequiresBody(t *testing.T) {
	...
	root := newTestRoot(f)
	root.SetArgs([]string{"order", "create"})
	err := root.Execute()
