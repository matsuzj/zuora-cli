...
- **実装メモ(2026-06-11, P2-4)**: pkg/cmdtest 新設(Run / OK / Reasons / Status + fixtures/ 雛形)。Run は root.go と同一の8 persistent flag を持つスタブ root + httptest + NewTestFactory を1呼び出しに(handler nil でバリデーション系も可)。OK は method/path の任意 assert 付き(既存テストのリクエスト形状 assert を保てる署名)。既存テストの移行は計画どおり P3-4。probe テストは main 現行 API(明示 WithCheckSuccess)— #71 マージ後に1行調整。**P2-4 完了(ハーネス部)**。
