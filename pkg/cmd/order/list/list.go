...
// and, with --pending, list-pending commands (#454).
...
--subscription, --subscription-owner, or --invoice-owner. With --subscription,
add --pending to list only that subscription's pending orders.`,
		Example: `  zr order list
  zr order list --status Completed
  zr order list --subscription A-S00000001
  zr order list --subscription A-S00000001 --pending
  zr order list --invoice-owner A00000001 --json`,
		Flags: []listcmd.Flag{
			...
			{Name: "pending", Bool: true, Usage: "With --subscription, list only pending orders"},
		...
		Path: func(args []string, flags map[string]string) string {
			...
			case flags["subscription"] != "":
				if flags["pending"] == "true" {
					return fmt.Sprintf("/v1/orders/subscription/%s/pending", url.PathEscape(flags["subscription"]))
				}
	...
	// its RunE to reject invalid scope-flag combinations before the request.
	// (RunE runs after the root's PersistentPreRunE, so the client is already
	// wired.)
	inner := cmd.RunE
	cmd.RunE = func(c *cobra.Command, args []string) error {
		...
		if pending, _ := c.Flags().GetBool("pending"); pending {
			if sub, _ := c.Flags().GetString("subscription"); sub == "" {
				return fmt.Errorf("--pending requires --subscription")
			}
		}
		return inner(c, args)
	}
