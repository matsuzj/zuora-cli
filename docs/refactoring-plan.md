...
- **実装メモ(2026-06-11)**: 6コミットで実装完了(抽出+characterization → rootCmd導出 → shlex → Load警告+パス一本化 → set側ガード → E2E)。計画との差分: ガードは予約名+自己参照に加え**不正クォートと空展開も set 時に拒否**(broken エイリアスを作れなくした)。`alias delete` は許容のまま(テストで固定)。e2e-local 35/35。`internal/config.Dir()` の最終呼び出しが消えた(P4-1 で削除可)。**P1-3 完了**。
