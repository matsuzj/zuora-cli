...
1. **この概要ファイルを読む**: 共通パターン・設計方針を把握
2. **Phase ファイルを確認**: 対象 Phase のファイル (`phase-XX-*.md`) で `- [x]` の項目を確認し、次に実装すべきタスクを特定
3. **依存関係を確認**: Phase 依存関係を確認し、前提 Phase が完了しているか検証
4. **API 仕様を参照**: 各コマンドの Zuora API パスが Phase ファイルに記載されている。詳細なリクエスト/レスポンス仕様は https://developer.zuora.com/v1-api-reference/api.md を `WebFetch` で取得して確認
5. **既存コードを読む**: 既に実装済みのコマンド (例: `pkg/cmd/account/list/`) のパターンを踏襲して新コマンドを実装
6. **テスト実行**: `task test` で既存テストが壊れていないことを確認してから新コードを追加
7. **Sandbox 検証**: 実装後は `zr auth login` で Sandbox に接続し、実際の API 呼び出しで動作確認

### Phase 一覧

| Phase | ファイル | 内容 |
|-------|----------|------|
| 0 | [phase-00-foundation.md](phase-00-foundation.md) | プロジェクト基盤 |
| 1 | [phase-01-auth-config.md](phase-01-auth-config.md) | 認証 + 設定 + Raw API |
| 2 | [phase-02-read-output.md](phase-02-read-output.md) | 出力フォーマッタ + Account/Subscription Read |
| 3 | [phase-03-write-contact.md](phase-03-write-contact.md) | Account/Subscription Write + Contact + SignUp |
| 4 | [phase-04-order.md](phase-04-order.md) | Order CRUD + ライフサイクル |
| 5 | [phase-05-commerce.md](phase-05-commerce.md) | Product / Plan / Charge |
| 6 | [phase-06-invoice-payment.md](phase-06-invoice-payment.md) | Invoice + Payment |
| 7 | [phase-07-usage-meter.md](phase-07-usage-meter.md) | Usage + Meter |
| 8 | [phase-08-ramp-commitment.md](phase-08-ramp-commitment.md) | Ramp + Commitment + Fulfillment + Prepaid |
| 9 | [phase-09-zoql-changelog.md](phase-09-zoql-changelog.md) | ZOQL + Subscription Change Log + Omnichannel |
| 10 | [phase-10-polish.md](phase-10-polish.md) | ポリッシュ + 配布 |
